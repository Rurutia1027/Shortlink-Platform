    ...
    <el-form ref="ruleFormRef" :model="formData" :rules="formRule" label-width="80px">
      <el-form-item label="Redirect URL" prop="originUrl">
        <el-input
            ...
            placeholder="Please enter a link starting with http:// or https:// or an app redirect URL"
        ></el-input>
      </el-form-item>
      ...
      <el-form-item>
        <div style="width: 100%; display: flex; justify-content: flex-end;">
          <el-button class="buttons" type="primary" @click="onSubmit(ruleFormRef)" :disabled="submitDisable">
            Confirm
          </el-button>
...
const props = defineProps({
  groupInfo: Array,
  isSingle: Boolean, // true for single, false for batch creation
})
...
// Query title from URL if description is empty
...
const formRule = reactive({
    ...
    {required: true, message: 'Please enter a description', trigger: 'blur'},
    {
      validator: function (rule, value, callback) {
        if (props.isSingle === false && describeRows.value !== originUrlRows.value) {
          callback(new Error('The number of descriptions must match the number of URLs'))
        }
  ...
  validDate: [{required: false, message: 'Please enter a date', trigger: 'blur'}],
})
